        ...
        <div class="table-wrapper">
          <table>
            ...
            <tbody>
              ...
              <tr v-else-if="filteredProducts.length === 0"><td colspan="8" class="empty">暂无库存商品</td></tr>
              <tr v-for="product in filteredProducts" :key="product.productId" v-else>
                ...
                <td class="product-time">{{ formatDate(product.lastReceiptTime) }}</td>
                <td class="product-time">
                  <div :class="{ 'text-danger': product.nextExpiryDaysLeft > 0 && product.nextExpiryDaysLeft <= 2, 'text-warning': product.nextExpiryDaysLeft > 2 && product.nextExpiryDaysLeft <= 5 }">
                    {{ formatDate(product.nextExpiryTime) }}
                  </div>
                  <div v-if="product.nextExpiryTime" class="product-sub">剩余{{ product.nextExpiryDaysLeft }}天</div>
                </td>
                ...
                <td>
                  <div class="action-btns">
                    <button class="btn btn-sm btn-primary" @click="quickRestock(product)" v-if="product.stockStatus !== 'NORMAL'">
                      快速补货
                    </button>
  ...
  try {
    ...
    const rows = await Promise.all(
      flowersRes.data.map(async (flower) => {
        ...
        const latestReceiptBatch = batches
          .filter(batch => batch?.receiptTime)
          .reduce((latest, batch) => {
            if (!latest) return batch
            const latestTime = new Date(latest.receiptTime).getTime()
            const nextTime = new Date(batch.receiptTime).getTime()
            if (Number.isNaN(latestTime)) return batch
            if (Number.isNaN(nextTime)) return latest
            return nextTime > latestTime ? batch : latest
          }, null)

        const earliestExpiryBatch = batches
          .map((batch) => {
            const availableQty = Number(batch.availableQty ?? (Number(batch.currentQty || 0) - Number(batch.lockedQty || 0)))
            return { batch, availableQty: Number.isFinite(availableQty) ? availableQty : 0 }
          })
          .filter(({ batch, availableQty }) => batch?.wiltTime && availableQty > 0)
          .reduce((earliest, current) => {
            if (!earliest) return current
            const earliestTime = new Date(earliest.batch.wiltTime).getTime()
            const currentTime = new Date(current.batch.wiltTime).getTime()
            if (Number.isNaN(earliestTime)) return current
            if (Number.isNaN(currentTime)) return earliest
            return currentTime < earliestTime ? current : earliest
          }, null)

        const lastReceiptTime = latestReceiptBatch?.receiptTime || ''
        const nextExpiryTime = earliestExpiryBatch?.batch?.wiltTime || ''
        const nextExpiryDaysLeft = nextExpiryTime ? calculateDaysLeft(nextExpiryTime) : 0

        const hasFefoWarning = batches.some(batch => {
          const daysLeft = calculateDaysLeft(batch.wiltTime)
          const availableQty = Number(batch.availableQty ?? (Number(batch.currentQty || 0) - Number(batch.lockedQty || 0)))
          return daysLeft <= 3 && availableQty > 0
        })
        ...
        const status = stockStatus === 'OUT_OF_STOCK'
          ? 'OUT_OF_STOCK'
          : stockStatus === 'WARNING' || hasFefoWarning
            ? 'WARNING'
            : 'NORMAL'
        const statusText = stockStatus === 'OUT_OF_STOCK'
          ? '缺货'
          : stockStatus === 'WARNING'
            ? '低库存'
            : hasFefoWarning
        ...
        return {
          productId: flower.id,
          productName: flower.name || ('Flower #' + flower.id),
          ...
          warnThreshold,
          suggestedRestock: stockStatus === 'NORMAL' ? 0 : Math.max(warnThreshold - availableStock, 0),
          stockStatus,
          hasFefoWarning,
          status,
          statusText
...
function updateStats() {
  stats.value.lowStockCount = products.value.filter(p => p.stockStatus === 'WARNING').length
  stats.value.outOfStockCount = products.value.filter(p => p.stockStatus === 'OUT_OF_STOCK').length
  stats.value.alertCount = products.value.filter(p => p.status !== 'NORMAL').length
  stats.value.totalRestockNeeded = products.value.reduce((sum, p) => sum + ((p.stockStatus === 'NORMAL' ? 0 : p.suggestedRestock) || 0), 0)
}
...
function exportAlertList() {
  const csvContent = [
    ['商品名称', '花材名称', '最近进货', '最早过期', '当前库存', '预警阈值', '建议补货量', '状态'].join(','),
    ...filteredProducts.value.map(p => [
      p.productName,
      p.flowerName || '',
      formatDate(p.lastReceiptTime),
      formatDate(p.nextExpiryTime),
      p.currentStock,
      p.warnThreshold,
      p.suggestedRestock || 0,
      p.statusText || getStatusText(p.status)
    ].join(','))
  ].join('\n')
