  ...
  <div class="login-shell">
    <div class="login-panel card single-panel">
      <div class="login-copy compact-copy">
        <p class="login-kicker">MERCHANT SECURITY</p>
        <h1 class="login-title">修改商家密码</h1>
        <p class="login-subtitle">未登录也可以直接修改商家密码，修改成功后请使用新密码重新登录。</p>
      </div>
      <form class="login-form" @submit.prevent="submitChange">
        ...
        <div class="form-group"><label>当前密码</label><input v-model="oldPassword" class="form-control" type="password" autocomplete="current-password" placeholder="请输入当前密码"></div>
        <div class="form-group"><label>新密码</label><input v-model="newPassword" class="form-control" type="password" autocomplete="new-password" placeholder="至少 8 位"></div>
        <div class="form-group"><label>确认新密码</label><input v-model="confirmPassword" class="form-control" type="password" autocomplete="new-password" placeholder="请再次输入新密码"></div>
        <div class="login-actions"><router-link class="login-link" to="/merchant/login">返回登录</router-link><router-link class="login-link" to="/merchant/register">注册账号</router-link></div>
        <div v-if="successMessage" class="login-notice">{{ successMessage }}</div>
        <div v-if="errorMessage" class="login-error">{{ errorMessage }}</div>
        <button class="btn btn-primary login-submit" :disabled="loading" type="submit">{{ loading ? '提交中…' : '确认修改' }}</button>
      </form>
    </div>
  </div>
</template>
...
const account = ref(getStoredAuth()?.account || 'merchant')
...
async function submitChange() {
  ...
  if (!account.value || !oldPassword.value || !newPassword.value || !confirmPassword.value) return (errorMessage.value = '请完整填写密码信息')
  if (newPassword.value.length < 8) return (errorMessage.value = '新密码至少需要 8 位')
  if (newPassword.value !== confirmPassword.value) return (errorMessage.value = '两次输入的新密码不一致')
  loading.value = true
  try {
    const res = await api.changePassword(account.value, oldPassword.value, newPassword.value)
    if (!res.success) return (errorMessage.value = res.message || '修改密码失败，请稍后重试')
    successMessage.value = res.message || '密码修改成功，请重新登录'
    clearStoredAuth()
    setTimeout(() => { router.replace({ path: '/merchant/login', query: { notice: 'password-updated' } }) }, 600)
  } catch {
