          ...
          <table>
            ...
            <tbody>
              ...
              <tr v-for="user in users" :key="user.id" v-else>
                ...
                <td>{{ user.phone || '-' }}</td>
                <td><span class="badge badge-warning">{{ user.points ?? 0 }}</span></td>
                <td style="font-size:12px;color:var(--text-muted)">{{ formatTime(user.created_at) }}</td>
                <td><button class="btn btn-sm btn-outline" @click="viewUser(user.id)">详情</button></td>
              </tr>
            </tbody>
          </table>
        </div>
            ...
            <table>
              ...
              <tbody>
                <tr v-for="order in userOrders" :key="order.orderNo">
                  ...
                  <td><span class="badge badge-info">{{ statusMap[order.status] || order.status }}</span></td>
