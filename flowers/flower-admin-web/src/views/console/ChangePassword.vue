  ...
  <div class="login-shell">
    <div class="login-panel card single-panel">
      <div class="login-copy compact-copy">
        ...
        <h1 class="login-title">修改管理员密码</h1>
        <p class="login-subtitle">未登录也可以直接修改密码，修改成功后请使用新密码重新登录。</p>
      </div>
      ...
      <form class="login-form" @submit.prevent="submitChange">
        ...
        <div class="form-group">
          <label>当前密码</label>
          <input v-model="oldPassword" class="form-control" type="password" autocomplete="current-password" placeholder="请输入当前密码">
        </div>
        <div class="form-group">
          <label>新密码</label>
          <input v-model="newPassword" class="form-control" type="password" autocomplete="new-password" placeholder="至少 8 位">
        </div>
        <div class="form-group">
          <label>确认新密码</label>
          <input v-model="confirmPassword" class="form-control" type="password" autocomplete="new-password" placeholder="请再次输入新密码">
        </div>
        ...
        <div v-if="successMessage" class="login-notice">{{ successMessage }}</div>
...
import { api, clearStoredAuth, getStoredAuth } from '../../api'

const router = useRouter()
const account = ref(getStoredAuth()?.account || 'admin')
...
async function submitChange() {
  ...
  if (!account.value || !oldPassword.value || !newPassword.value || !confirmPassword.value) {
  ...
  loading.value = true
  try {
    const res = await api.changePassword(account.value, oldPassword.value, newPassword.value)
    if (!res.success) {
      errorMessage.value = res.message || '修改密码失败，请稍后重试'
      return
    }

    successMessage.value = res.message || '密码修改成功，请重新登录'
    clearStoredAuth()
    setTimeout(() => {
      router.replace({ path: '/login', query: { notice: 'password-updated' } })
    }, 600)
  } catch {
...
.login-shell::before,
.login-shell::after { content: ''; position: absolute; inset: 0; pointer-events: none; }
.login-shell::before { inset: -12%; background: radial-gradient(circle at 12% 18%, rgba(255, 160, 196, 0.34), transparent 18%), radial-gradient(circle at 84% 22%, rgba(255, 209, 102, 0.2), transparent 20%), radial-gradient(circle at 70% 78%, rgba(103, 188, 145, 0.24), transparent 20%), radial-gradient(circle at 22% 82%, rgba(255, 255, 255, 0.4), transparent 16%); filter: blur(10px); transform: scale(1.08); z-index: 0; }
.login-shell::after { background-image: linear-gradient(rgba(255, 255, 255, 0.26) 1px, transparent 1px), linear-gradient(90deg, rgba(255, 255, 255, 0.2) 1px, transparent 1px); background-size: 34px 34px; mask-image: radial-gradient(circle at center, rgba(0, 0, 0, 0.9), transparent 88%); opacity: 0.5; z-index: 0; }
.login-panel { width: min(920px, 100%); position: relative; z-index: 1; display: grid; grid-template-columns: 1.1fr 0.9fr; gap: 28px; padding: 32px; border: 1px solid rgba(255, 255, 255, 0.7); background: linear-gradient(145deg, rgba(255, 255, 255, 0.78), rgba(255, 255, 255, 0.6)); backdrop-filter: blur(22px); box-shadow: 0 24px 60px rgba(173, 92, 122, 0.14), 0 10px 20px rgba(57, 86, 68, 0.08); }
