...
CAPTION = "图3-10 订单明细实体属性描述"


def build_html() -> str:
    attrs = ["明细编号", "订单编号", "商品编号", "商品名称", "单价", "数量", "小计金额", "创建时间"]
...
def render_png(out_png: Path) -> None:
    ...
    with tempfile.TemporaryDirectory(prefix="fig310cn_") as td:
        td_path = Path(td)
        html_file = td_path / "fig310.html"
...
def main() -> int:
    ...
    png = out_dir / "图3-10_订单明细实体属性描述_中文版.png"
    render_png(png)
